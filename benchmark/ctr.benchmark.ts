...
console.log(`Benchmarking CTR mode with ${DATA_SIZE / (1024 * 1024)} MB of data`)

// Crear instancia de HighLevelAES para CTR
const ctrCipher = new HighLevelAES(AesMode.CTR, key)

// Benchmark de cifrado
console.log("Running encryption benchmark...")
const encryptTime = measureTime(() => {
  const encrypted = ctrCipher.encrypt(testData, { iv })
})

console.log(formatResult("CTR", "encryption", encryptTime, DATA_SIZE))

// Usar los mismos datos cifrados para el benchmark de descifrado
const encryptedData = ctrCipher.encrypt(testData, { iv })

// Benchmark de descifrado
console.log("Running decryption benchmark...")
const decryptTime = measureTime(() => {
  const decrypted = ctrCipher.decrypt(encryptedData.ciphertext, { iv: encryptedData.iv })
})

console.log(formatResult("CTR", "decryption", decryptTime, DATA_SIZE))

console.log("CTR benchmark completed.")
