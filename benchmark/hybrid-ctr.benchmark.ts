...
console.log(`Benchmarking HybridCTR mode with ${DATA_SIZE / (1024 * 1024)} MB of data`)

// Crear instancia de HighLevelAES para HybridCTR
const hybridCtrCipher = new HighLevelAES(AesMode.HYBRID_CTR, key)

// Benchmark de cifrado
console.log("Running encryption benchmark...")
const encryptTime = measureTime(() => {
  const encrypted = hybridCtrCipher.encrypt(testData, { nonce })
})

console.log(formatResult("HybridCTR", "encryption", encryptTime, DATA_SIZE))

// Usar los mismos datos cifrados para el benchmark de descifrado
const encryptedData = hybridCtrCipher.encrypt(testData, { nonce })

// Benchmark de descifrado
console.log("Running decryption benchmark...")
const decryptTime = measureTime(() => {
  const decrypted = hybridCtrCipher.decrypt(encryptedData.ciphertext, {
    nonce: encryptedData.iv,
    tag: encryptedData.tag
  })
})

console.log(formatResult("HybridCTR", "decryption", decryptTime, DATA_SIZE))

console.log("HybridCTR benchmark completed.")
