...
// Nonce de 12 bytes para GCM-SIV
...
console.log(`Benchmarking GCM-SIV mode with ${DATA_SIZE / (1024 * 1024)} MB of data`)

// Crear instancia de HighLevelAES para GCM-SIV
const gcmSivCipher = new HighLevelAES(AesMode.GCM_SIV, key)

// Benchmark de cifrado
console.log("Running encryption benchmark...")
const encryptTime = measureTime(() => {
  const encrypted = gcmSivCipher.encrypt(testData, { nonce })
})

console.log(formatResult("GCM-SIV", "encryption", encryptTime, DATA_SIZE))

// Usar los mismos datos cifrados para el benchmark de descifrado
const encryptedData = gcmSivCipher.encrypt(testData, { nonce })

// Benchmark de descifrado
console.log("Running decryption benchmark...")
const decryptTime = measureTime(() => {
  const decrypted = gcmSivCipher.decrypt(encryptedData.ciphertext, {
    nonce: encryptedData.iv,
    tag: encryptedData.tag
  })
})

console.log(formatResult("GCM-SIV", "decryption", decryptTime, DATA_SIZE))

console.log("GCM-SIV benchmark completed.")
