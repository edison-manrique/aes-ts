...
// Nonce de 12 bytes para OCB (debe ser entre 1 y 15 bytes)
...
console.log(`Benchmarking OCB mode with ${DATA_SIZE / (1024 * 1024)} MB of data`)

// Crear instancia de HighLevelAES para OCB
const ocbCipher = new HighLevelAES(AesMode.OCB, key)

// Benchmark de cifrado
console.log("Running encryption benchmark...")
const encryptTime = measureTime(() => {
  const encrypted = ocbCipher.encrypt(testData, { nonce })
})

console.log(formatResult("OCB", "encryption", encryptTime, DATA_SIZE))

// Usar los mismos datos cifrados para el benchmark de descifrado
const encryptedData = ocbCipher.encrypt(testData, { nonce })

// Benchmark de descifrado
console.log("Running decryption benchmark...")
const decryptTime = measureTime(() => {
  const decrypted = ocbCipher.decrypt(encryptedData.ciphertext, {
    nonce: encryptedData.iv,
    tag: encryptedData.tag
  })
})

console.log(formatResult("OCB", "decryption", decryptTime, DATA_SIZE))

console.log("OCB benchmark completed.")
