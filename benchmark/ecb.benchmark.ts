...
console.log(`Benchmarking ECB mode with ${DATA_SIZE / (1024 * 1024)} MB of data`)

// Crear instancia de HighLevelAES para ECB
const ecbCipher = new HighLevelAES(AesMode.ECB, key)

// Benchmark de cifrado
console.log("Running encryption benchmark...")
const encryptTime = measureTime(() => {
  const encrypted = ecbCipher.encrypt(testData)
})

console.log(formatResult("ECB", "encryption", encryptTime, DATA_SIZE))

// Usar los mismos datos cifrados para el benchmark de descifrado
const encryptedData = ecbCipher.encrypt(testData)

// Benchmark de descifrado
console.log("Running decryption benchmark...")
const decryptTime = measureTime(() => {
  const decrypted = ecbCipher.decrypt(encryptedData.ciphertext)
})

console.log(formatResult("ECB", "decryption", decryptTime, DATA_SIZE))

console.log("ECB benchmark completed.")
