...
console.log(`Benchmarking GCM mode with ${DATA_SIZE / (1024 * 1024)} MB of data`)

// Crear instancia de HighLevelAES para GCM
const gcmCipher = new HighLevelAES(AesMode.GCM, key)

// Benchmark de cifrado
console.log("Running encryption benchmark...")
const encryptTime = measureTime(() => {
  const encrypted = gcmCipher.encrypt(testData, { iv })
})

console.log(formatResult("GCM", "encryption", encryptTime, DATA_SIZE))

// Usar los mismos datos cifrados para el benchmark de descifrado
const encryptedData = gcmCipher.encrypt(testData, { iv })

// Benchmark de descifrado
console.log("Running decryption benchmark...")
const decryptTime = measureTime(() => {
  const decrypted = gcmCipher.decrypt(encryptedData.ciphertext, {
    iv: encryptedData.iv,
    tag: encryptedData.tag
  })
})

console.log(formatResult("GCM", "decryption", decryptTime, DATA_SIZE))

console.log("GCM benchmark completed.")
