...
console.log(`Benchmarking CBC mode with ${DATA_SIZE / (1024 * 1024)} MB of data`)

// Crear instancia de HighLevelAES para CBC
const cbcCipher = new HighLevelAES(AesMode.CBC, key)

// Benchmark de cifrado
console.log("Running encryption benchmark...")
const encryptTime = measureTime(() => {
  const encrypted = cbcCipher.encrypt(testData, { iv })
})

console.log(formatResult("CBC", "encryption", encryptTime, DATA_SIZE))

// Usar los mismos datos cifrados para el benchmark de descifrado
const encryptedData = cbcCipher.encrypt(testData, { iv })

// Benchmark de descifrado
console.log("Running decryption benchmark...")
const decryptTime = measureTime(() => {
  const decrypted = cbcCipher.decrypt(encryptedData.ciphertext, { iv: encryptedData.iv })
})

console.log(formatResult("CBC", "decryption", decryptTime, DATA_SIZE))

console.log("CBC benchmark completed.")
