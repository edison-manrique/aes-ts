...
// Tamaño de datos para benchmark (1 MB) lento
...
console.log(`Benchmarking CFB mode with ${DATA_SIZE / (1024 * 1024)} MB of data`)

// Crear instancia de HighLevelAES para CFB
const cfbCipher = new HighLevelAES(AesMode.CFB, key)

// Benchmark de cifrado
console.log("Running encryption benchmark...")
const encryptTime = measureTime(() => {
  const encrypted = cfbCipher.encrypt(testData, { iv })
})

console.log(formatResult("CFB", "encryption", encryptTime, DATA_SIZE))

// Usar los mismos datos cifrados para el benchmark de descifrado
const encryptedData = cfbCipher.encrypt(testData, { iv })

// Benchmark de descifrado
console.log("Running decryption benchmark...")
const decryptTime = measureTime(() => {
  const decrypted = cfbCipher.decrypt(encryptedData.ciphertext, { iv: encryptedData.iv })
})

console.log(formatResult("CFB", "decryption", decryptTime, DATA_SIZE))

console.log("CFB benchmark completed.")
